...
if(!empty($_REQUEST['eid']) && intval($_REQUEST['eid']) > 0 ){
	...
	$query = "SELECT * FROM mon_expedition WHERE id={$EXPEDITION_ID} AND  status IN (20, 21, 22,  30) ORDER BY date_start DESC , mni_name";
	$arResult['EXPEDITION'] = $DB->getRecord($query);
	if(!empty($arResult['EXPEDITION'])){
