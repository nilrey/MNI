...
//		$exp_year = $date->format('Y');
	...
	<tr onmouseover="this.style.background='#E6E6FA'" onmouseout="this.style.background='#fff'">
		<td onclick="window.location = 'editgroup.php?eid=<?=$arItem['id']?>' "><center><?=$arItem['id']?></center></td>
		<td onclick="window.location = 'editgroup.php?eid=<?=$arItem['id']?>' "><?=$arItem['name']?></td>
		<td onclick="window.location = 'editgroup.php?eid=<?=$arItem['id']?>' "><center><?=$arItem['active']?></center></td>
	</tr>
	<?}?>
</table>
